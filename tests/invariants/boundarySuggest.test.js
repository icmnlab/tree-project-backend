/**
 * invariants/boundarySuggest.test.js — 建議邊界 outlier 排除邏輯（純邏輯，免 DB）
 */
'use strict';
...
module.exports = {
    ...
    cases: [
        {
            name: 'boundarySuggest: 主群集 + 一個遠距 outlier → 排除 outlier',
            run: async () => {
                const trees = [
        ...
        {
            name: 'boundarySuggest: suggestBoundaryFromTrees 成功產生 coordinates',
            run: async () => {
                const trees = [
        ...
        {
            name: 'boundarySuggest: 樹木不足 3 棵 → INSUFFICIENT_TREES',
            run: async () => {
