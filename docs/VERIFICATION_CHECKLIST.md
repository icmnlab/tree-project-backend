...
> 更新：2026-06-15  
> 適用版本：前端 `18.10.4+26`；資料庫 migration 應 **≥ 35**（28 儀器溯源、30 邊界來源、31 樹木生命週期、32 照片綁定量測、33 樹況選單目錄、34 亂碼防線、35 生命週期回填對齊；見 `DATABASE_NORMALIZATION.md`）
...
> ⚠️ 後端位址請填自己的主機（文件一律用 `<SERVER_IP>`／`<TAILSCALE_HOST>` 占位符）。

> 🧭 **座標 SOP（資料正確性關鍵）**：取 GPS 時務必**人站在樹旁**取點（拿儀器或手機皆可），系統一律把座標視為**樹位**、不再以 HD/AZ 偏移。
